...
public class UseRefreshTokenUseCaseTest
{
    ...
    public async Task Error_RefreshToken_Expired()
    {
        (var user, _) = UserBuilder.Build();
        var refreshToken = RefreshTokenBuilder.Build(user);
        refreshToken.CreatedOn = DateTime.UtcNow.AddDays(-StandardProjectRuleConstants.REFRESH_TOKEN_EXPIRATION_DAYS - 1);
    ...
    private static UseRefreshTokenUseCase CreateUseCase(StandardProject.Domain.Entities.RefreshToken? refreshToken = null)
    {
